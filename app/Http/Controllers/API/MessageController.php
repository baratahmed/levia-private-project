...
class MessageController extends Controller
{
    ...
    public function sendMessage(Request $request){
        $this->validate($request, [
            ...
            'rest_id' => 'sometimes|exists:rest_info,id',
            'opponent_user_id' => 'sometimes|exists:user_info,id',
        ...
        if ($request->has('order_id')){
            ...
            if ($request->has('to') && strtoupper($request->input('to')) === "DR"){ // Conversation between this restaurant and the DR
                // return ("TO DR");
                $conversation = Conversation::byOrderUserUser($order, $user, $dr);
            ...
            $rest = RestaurantInfo::findOrFail($request->input('rest_id'));

            $conversation = Conversation::byRestAndUser($rest, $user);
        }
        else if ($request->has('opponent_user_id')){
            $rest = User::findOrFail($request->input('opponent_user_id'));

            $conversation = Conversation::byUserAndUser($rest, $user);
        }
            ...
            return response([
                'success' => false,
                'message' => 'Please provide the associated order_id, rest_id or user_id with the message.',
            ]);
        }
    ...
    public function getConversationDetails(Request $request){
        ...
        if ($request->has('order_id')){
            ...
            if ($request->has('to') && strtoupper($request->input('to')) === "DR"){ // Conversation between this restaurant and the DR
                // return ("TO DR");
                $conversation = Conversation::byOrderUserUser($order, $user, $dr, false);
            } else { // Conversation between this restaurant and the customer
                // return ("TO USER");
                $conversation = Conversation::byOrderRestUser($order, $rest, $user, false);
            }
        }
        else if ($request->has('rest_id')){
            $rest = RestaurantInfo::findOrFail($request->input('rest_id'));

            $conversation = Conversation::byRestAndUser($rest, $user, false);
        }
        else if ($request->has('opponent_user_id')){
            $rest = User::findOrFail($request->input('opponent_user_id'));

            $conversation = Conversation::byUserAndUser($rest, $user, false);
        }
        else if ($request->has('conversation_id')){
            $conversation = Conversation::findOrFail($request->input('conversation_id'));
