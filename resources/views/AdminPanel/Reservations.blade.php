                ...
                <table class="table">
                    ...
                    @foreach($reservations as $reserve)
                        <tr {!! $reserve->is_seen == false ? 'style="background:#efafa2"' : '' !!}>
                            ...
                            <?php 
                            $user = \App\Models\User::findOrFail($reserve->user_id);
                            $rest = \App\Models\RestaurantInfo::findOrFail($reserve->rest_id); 
                            ?>
                            <td scope="row" >{{ $rest->rest_name }} | ID: {{ $rest->id }}</td>
                            ...
                            <td scope="row" >{{ $reserve->checked_in_time != null ? \Carbon\Carbon::parse($reserve->checked_in_time)->toDayDateTimeString() : "N/A" }}</td>
...
@section('extra-js')
    ...
    <script>
        ...
        $(document).on('click', '.accept-button', function(e){
            ...
            $.ajax({
                method: "POST",
                url: "{{ route('admin.acceptReservation') }}",
