...
@section('content')
    ...
    <form method="post" action="{{route('admin.postEditOffer', $offer->offer_id)}}" enctype="multipart/form-data">
        {{ csrf_field() }}
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <select class="form-control" name="rest_id" id="choose_restaurant">
                            ...
                            @foreach ($restaurants as $restaurant)
                                <option value="{{ $restaurant->id }}" {{ $offer->rest_id == $restaurant->id ? 'selected' : '' }} >{{ $restaurant->rest_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="restaurant-name">Title</label>
                        <input type="text" value="{{ $offer->offer_title }}" name="offer_title" id="restaurant-name" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label for="restaurant-logo">Picture</label>
                        <input type="file" class="rest_image_url" name="offer_image" id="restaurant-logo">
                        <br />
                        <img style="height:100px; width:auto; border:1px solid black;" src="{{ $offer->imageUrl }}" alt="{{ $offer->offer_title }}">
                    </div>
                    <div class="form-group">
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <select class="form-control" name="offer_type_id" id="restaurant-address-district" required>
                                    @foreach ($offer_type as $type)
                                        <option value="{{ $type->offer_type_id }}" {{ $type->offer_type_id == $offer->offer_type_id ? 'selected' : '' }}>{{ $type->offer_type_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-4">
                                <label for="restaurant-address-road-no">Start</label>
                                <input type="date" value="{{ $offer->offer_starting_date }}" name="offer_starting_date" id="restaurant-address-road-no" class="form-control" required>
                            </div>
                            <div class="col-md-4">
                                <label for="restaurant-address-police-station">End</label>
                                <input type="date" value="{{ $offer->offer_ending_date }}" name="offer_ending_date" id="restaurant-address-police-station" class="form-control" required>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="offer_desc" id="restaurant-name" class="form-control" rows="5">{{ $offer->offer_desc }}</textarea>
                    </div>

                    <div class="form-group">
                        <label for="restaurant-logo">Terms and Condition</label>
                        <textarea name="offer_tc" id="restaurant-name" class="form-control" rows="5">{{ $offer->offer_tc }}</textarea>
                    </div>
                    ...
                    <div class="form-group specific_food_box" style="{{ $offer->food_id === null ? "display:none;" : "" }}">
                        ...
                        <select class="form-control" name="food_id" id="choose_food">
                            ...
                            @php
                                $food_details = \App\Models\RestFoodDetailsDataset::for($offer->rest_id)->get();
                            @endphp
